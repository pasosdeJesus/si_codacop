...
if (test "${CONFIG_HOSTS}" = "") then {
  echo "Definir variable de ambiente CONFIG_HOSTS"
  exit 1;
} fi;
...
${defuroot} PUERTOUNICORN=${PUERTOUNICORN} RUTA_RELATIVA=${RUTA_RELATIVA} CONFIG_HOSTS=${CONFIG_HOSTS} DIRAP=$DIRAP RAILS_ENV=production SECRET_KEY_BASE=${SECRET_KEY_BASE} bundle exec /usr/local/bin/unicorn_rails -c $DIRAP/config/unicorn.conf.minimal.rb  -E production -D"
